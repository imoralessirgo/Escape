...
public class HexPathFind {
		...
		for (i = 0; i < distance; i++) {
			...
			switch (pt.getMovementPattern()) {
				...
				case LINEAR: // problematic
					for(int[] dir : hex) {
						if(checkLinear((HexCoordinate) from, dir, distance) == true) {
							return true;
						}
					}
					return false;
	...
	private static boolean checkLinear(HexCoordinate sc, int[] direction,
		...
		for (int j = 0; j < maxDistance; j++) {
			sc = HexCoordinate.makeCoordinate(sc.getX() + direction [0], sc.getY() + direction[1]);
	...
	private static boolean linearIsValid(HexCoordinate sc, int[] dir) {
		...
		if (sb.getPieceAt(sc) != null) {
			if (canFly()) {
				...
				HexCoordinate next = HexCoordinate.makeCoordinate(sc.getX() + dir [0], sc.getY() + dir[1]);
	 ...
	 * @param c
	 */
	private static boolean jumpFromLinear(HexCoordinate curr, HexCoordinate neighbour) {
		HexBoard sb = (HexBoard) board;
		if (sb.getLocationType(curr) == LocationType.EXIT) {
			// only valid to fly over or end at an exit
			if (canFly() || dest.equals(neighbour)) {
			...
			// trying to move to a blockeOrthoSquareCoordinated position without unblock capabilities
			// is invalid
			if ((canFly() || canUnblock()) && !dest.equals(neighbour)) {
				return true;
			} else
